...
export default function GuideSearch(props) {
    return (
        <div className="search-box p-5 passion-radio rounded-2xl bg-black-400 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-10">
            <nav className="guide-cat mb-10 lg:hidden">
                <ul className="flex gap-2 md:gap-6">
                    {
                        props.cat.map((item, index) => {
                            return (
                                <li key={index} className="cat flex-1">
                                    <a className="py-1 pt-[0.45rem] px-1 block rounded-full bg-light-red text-center text-[0.65rem] md:text-base font-semibold text-black tracking-tight" href={item.url}>{item.title}</a>
                                </li>
                            );
                        })
                    }
            ...
            <p className="text-center text-xs font-medium mb-10 lg:mb-4">Search our events calendar and find the perfect club nights and events to attend during your stay in Ibiza!</p>
            <ul className="months gap-3 mb-4 hidden lg:flex">
                {
                    month.map((value, index) => {
                        return (
                            <li key={index} className="flex-1">
                                <a href="#" className="block p-1 pt-[0.35rem] bg-light-red rounded-full text-xs text-center text-black">{value}</a>
                            </li>
                        );
                    })
                }
