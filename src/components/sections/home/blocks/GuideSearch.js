...
export default function GuideSearch(props) {
  return (
    <div className="search-box p-5 passion-radio rounded-2xl bg-black-400 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-10">
          ...
          {props.cat.map((item, index) => {
            return (
              <li key={index} className="cat flex-1">
                <a
                  className="py-1 pt-[0.45rem] px-1 block rounded-full bg-light-red text-center text-[0.65rem] md:text-base font-semibold text-black tracking-tight"
                  href={item.url}
                >
                  {item.title}
                </a>
      ...
      <p className="text-center text-xs font-medium mb-10 lg:mb-4">
      ...
      <div className="flex flex-col lg:flex-row gap-4 text-sm">
        <div className="genre flex-1">
          {/*<Select options={genre} styles={customStyles} /> */}
        </div>
        <div className="venue flex-1">
          {/* <Select options={venue} styles={customStyles} /> */}
        </div>
        <div className="type flex-1">
          {/*<Select options={type} styles={customStyles} /> */}
        </div>
        <div className="type flex-1 lg:hidden">
          {/*<Select options={mnt} styles={customStyles} /> */}
        </div>
